...
@Component
public class ContractRenewalWorkflowCompletionHandler implements WorkflowCompletionHandler {
  private static final Logger log = LoggerFactory.getLogger(ContractRenewalWorkflowCompletionHandler.class);

  private final ContractChangeService contractChangeService;

  public ContractRenewalWorkflowCompletionHandler(@Lazy ContractChangeService contractChangeService) {
  ...
  public void onCompleted(WorkflowInstanceEntity instance) {
    ...
    log.info("Contract renewal workflow completed: requestId={}, instanceId={}", requestId, instance.getId());
  ...
  public void onRejected(WorkflowInstanceEntity instance) {
    long requestId = parseRequestId(instance.getBusinessId());
    log.info("Contract renewal workflow rejected: requestId={}, instanceId={}", requestId, instance.getId());
    contractChangeService.onWorkflowRejected(requestId);
  }

  private static long parseRequestId(String businessId) {
    if (businessId == null || businessId.isBlank()) {
      throw new IllegalArgumentException("合同续签流程 businessId 为空");
    }
    try {
